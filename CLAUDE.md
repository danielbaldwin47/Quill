...
An `/implement` session whose ticket has no Hand test (Ticket tier only) lands its own work: once the Gate is green and `/code-review` is done, `tools/land <PR> <ticket>` merges the PR (the body's `Closes #N` closes the ticket), pulls `main`, runs the Commit tier on it, removes the worktree and its branches, and posts the `tools/context-report` line on the ticket. A green ticket PR lands without asking — the owner's standing instruction (2026-09-11) — and only a Hand test hands the close to the owner.

## Context in an `/implement` session

The smart zone is about 120k tokens, and every tool call leaves about 1.5k in context for the rest of the session (`docs/agents/context.md` has the measurements), so the zone is held by making fewer calls, not smaller ones. The session is these eight steps in order, each ending on the state it names.

1. **Ticket.** `tools/ticket <N>` once: the ticket and its parent spec land as two files under the job's tmp directory, each with a heading index, and every later question about either is a `sed -n` range on those files (`tools/ticket --graph <M>` is a spec's children on one screen). Done when both files exist.
2. **Orientation.** One `Agent` call with `subagent_type: "fork"` before any edit — a fork is the one agent type that inherits the `LSP` tool; the built-in types answer every symbol question with `grep`. Its prompt tells it to load the tool with `ToolSearch` (`select:LSP`), to `sleep 20` and retry the one empty first call, to take every Rust symbol question through it (§ Rust), and to return `file:line` ranges. This context reads those ranges and nothing wider. A ticket naming one module reads that module by `sed -n` range instead (#229, the cheapest session). Done when the ranges are in hand; the step runs again after a compaction and before a hand-fix pass.
3. **Worktree.** `EnterWorktree` (`docs/agents/worktree.md`: the refusal list in full, and how an orchestrator shares worktrees with its forks). From here Bash is plain words: a command that needs a `$VAR`, `$(…)`, heredoc or loop is a script made with Write and run as `bash <absolute path>`; `git` stands alone in its command with every path spelled out; memory files change through Edit.
4. **Edit.** Files are made with Write and changed with Edit, in rustfmt's shape so `cargo fmt` changes nothing: a file the session has opened and then changes through Bash comes back into context as a diff snippet, and `.claude/hooks/edited-files-guard.sh` refuses the in-place shapes. While iterating: `cargo clippy --all-targets -q --message-format=short -- -D warnings` (the Gate's own step, under a second on a checked tree) and `cargo test <name>`. Docs by section: `docs/agents/gate.md` for the tier the ticket names, `docs/architecture.md` for the sections the ticket cites, ADRs by number; `docs/agents/hand-tests.md` is `/to-spec`'s reading.
5. **Gate.** `tools/gate check` is the whole Commit tier in one result. Done when it ends in `pass`.
6. **Judge.** For a judged Piece: `tools/gate shoot <piece>` in the foreground to look, `tools/gate judge <piece>` in the background (`run_in_background`, then end the turn — it outlasts the foreground cap, `docs/agents/gate.md` § As run), `tools/gate bench` in the foreground; each is read for its summary lines, and the shots are the critic's to look at. Done when the round is ours, or the ticket's brief says what ours cannot render.
7. **Review.** `/code-review` (§ Code review), every finding applied or answered on the PR.
8. **Land.** `tools/land <PR> <ticket>` (§ Gate), or the Hand test handed to the owner.

...
A long-form writing environment for Linux: a native GTK4 app in Rust, ported Piece by Piece from the JavaScript app that won its blind gauntlet against iA Writer. That app is the **Parity oracle**, retired from the tree once every Piece was ported: its frozen shots under `dev/shots/oracle/` stay the opponent for the states still paired with it, and the states `docs/design.md` hands over are judged against the **Design oracle** — iA Writer for Mac as measured in `dev/ref/ia/mac-native/` (ADR 0015). `docs/architecture.md` specifies the native app: an `/implement` session reads the sections its ticket cites (§ Context in an `/implement` session); any other Rust, packaging or repo work reads it first. Its Gate is `docs/agents/gate.md`, and `CONTEXT.md` holds the vocabulary both use.
...
- `tools/` — the Gate's command and its helpers: `tools/gate --help` names its subcommands and what each ends in, `docs/agents/gate.md` says which tier requires which, and a selftest beside each subcommand keeps it honest. `npm i` at the root once, for the ones that drive a browser. Four are a ticket's: `tools/ticket <N>` (§ Context in an `/implement` session), `tools/land <PR> <ticket>` (the whole landing sequence in one command, § Gate), `context-report <N>` (what a landed ticket's `/implement` session cost, in the one line its closing comment carries) and `context-report-selftest` (which keeps that honest against a written-out transcript); `run-cost <transcript>...` is what any agent run cost in API dollars, read from a Claude Code or Codex transcript, with `run-cost-selftest` beside it. `ink-coverage.mjs` reads how much ink a judged crop lays down against the oracle's crop, for a rasterisation question a critic should not be spent on (#212).
- `dev/legacy/` — retired: the JavaScript app is in git history at the tag `legacy-last`, and a path under it that code or a round cites is read there (`git show legacy-last:<path>`). `dev/README.md` § The Parity oracle says how to restore a copy.
- `dev/` — everything the judging uses and nothing the package ships; `dev/README.md` says what each folder is.
- `dev/ref/ia/` — iA's fonts, and as evidence its templates and marketing stills; `dev/ref/ia/mac-native/` is the Design oracle as measured (`VERDICTS.md`, `NOTES.md`, the captures under `dev/ref/ia/shots/mac-native/`), and it outranks every still. `dev/ref/sample.md` is the shared test passage, and `dev/ref/short.md` the one that fits inside a window.
- `dev/shots/` and `dev/progress/` — judging evidence: per-Piece screenshots, round verdicts, latency JSON and the report. `dev/shots/oracle/` is the judged states (`states.json`) and the Parity oracle frozen at them, or the `mac-native` crop a state names; `dev/shots/blind/` is the critic's copy of shots held elsewhere, ignored by git. The shots and captures are **evidence**, kept in the main checkout alone and absent from a clone, except what the Gate reads to pass; `tools/gate` links them into a worktree and `tools/land` copies a worktree's new ones back (`dev/README.md` § Judging evidence).
- `PKGBUILD` + `packaging/` — Arch package of the native binary; `README.md` § Install has the build and install commands, and `dev/README.md` § Building the package the traps they avoid. The `.desktop` file and the icon are named for the application id, `io.github.danielbaldwin47.Quill`.
- `docs/` — `architecture.md` (the native spec), `design.md` (what the writing surface is meant to do where the Design oracle and the Parity oracle disagree — read its row before a caret, selection, palette, focus, markup or type ticket — `tools/design-rows` names every row with its line, and `grep -n '^| <Row> |' docs/design.md` reads the one — and its § Adding or changing a row before a spec), `shortcuts.md` (the one shortcut table every menu, the Palette and the shortcuts window read), `adr/` (decisions), `agents/` (the docs that § Gate and § Agent skills point at).

Licences: GPL-3.0-or-later at the root (`LICENSE`), OFL-1.1 for `fonts/`, Apache-2.0 for the bundled `harper-brill` tagger (`packaging/harper-brill-LICENSE`), MIT, BSD-3-Clause and CC0-1.0 for the entries `data/style/` took from others (`data/style/SOURCES.md`), and iA's own terms for `dev/ref/ia/`.

Workspace 1 is the user's: a test window (GTK, browser, bench) goes to a virtual output — `hyprctl output create headless`, as `tools/harness.mjs` `openStage` does — or to workspace 5, `hyprctl repl 'return hl.dispatch(hl.dsp.exec_cmd("[workspace 5 silent] <cmd>"))'`. Hyprland 0.56 parses dispatches as Lua, so the old `hyprctl dispatch exec` string form fails.

...
A long-form writing environment for Linux: a native GTK4 app in Rust, ported Piece by Piece from the JavaScript app that won its blind gauntlet against iA Writer. That app is now `dev/legacy/`, kept as the **Parity oracle** every native Piece is judged against until the last one is won, except the states `docs/design.md` hands to the **Design oracle** — iA Writer for Mac as measured in `dev/ref/ia/mac-native/` (ADR 0015). `docs/architecture.md` specifies the native app: an `/implement` session reads the sections its ticket cites (§ Context in an `/implement` session); any other Rust, packaging or repo work reads it first. Its Gate is `docs/agents/gate.md`, and `CONTEXT.md` holds the vocabulary both use.
...
- `dev/legacy/` — the JavaScript app as it won, and the Parity oracle: `dev/legacy/bin/quill` opens it from the checkout, `dev/legacy/tools/` shoots and benches it (`npm i` inside `dev/legacy/` too), `dev/legacy/BRIEF.md` and `dev/legacy/NOTES.md` describe it. It is ISC, under its own `dev/legacy/LICENSE`. Its `app/` and `tools/shoot.mjs` still name `ref/` and `shots/` at the root, from before `dev/`: every frozen oracle's fingerprint hashes those bytes, so a path fix there restales them all.
...
Licences: GPL-3.0-or-later at the root (`LICENSE`), ISC in `dev/legacy/`, OFL-1.1 for `fonts/`, Apache-2.0 for the bundled `harper-brill` tagger (`packaging/harper-brill-LICENSE`), MIT, BSD-3-Clause and CC0-1.0 for the entries `data/style/` took from others (`data/style/SOURCES.md`), and iA's own terms for `dev/ref/ia/`.

Workspace 1 is the user's: a test window (GTK, browser, bench) goes to a virtual output or to workspace 5, by the commands in `dev/legacy/BRIEF.md` § Headed windows. The same file, § Architecture, carries the glyph-advance rule every edit under `dev/legacy/app/` obeys.

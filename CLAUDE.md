...
- **A turn ends by replying with no tool call — that ending is the only wait.** A background agent's report, and a backgrounded command's exit, arrive as task notifications only after the reply finishes without a tool use; every tool call — even a no-op `echo` or a log `tail` — continues the same turn, so no call yields it and none brings the report sooner. The turn that launches the work does everything independent of the report — the commit, the push, the PR body — then names what it awaits in one line, and that line is the reply's end; every turn that opens before the notification is that one line and nothing else.

...
- **The ticket is fetched once**, with its parent spec: `tools/ticket <N>` writes both to one file under the job's tmp directory and prints the spec's headings with their line numbers, and later questions are answered from that file by `sed -n` range.

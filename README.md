...
files of one package with `duplicate target`. The filter drops the `quill-writer-debug` split package. And
...
The package is `quill-writer`, because `quill` in Arch's `extra` repository is a C++ logging library;
the command it installs is still `quill`. Its version carries the commit it was built from —
`quill-writer 0.1.0.r126.g92d3c1e` is `92d3c1e` — so `pacman -Q quill-writer` says which build is installed. Build from the checkout you mean to test: a

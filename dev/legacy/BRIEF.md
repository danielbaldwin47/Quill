...
`app/index.html` + `app/css/*.css` + `app/js/*.js`. Serve with `node dev/legacy/tools/serve.mjs 4173` (usually already running at http://localhost:4173/).
...
## Tools
- Screenshot: `node dev/legacy/tools/shoot.mjs --out dev/shots/<piece>/x.png --w 1440 --h 900 --dpr 2 --theme light|dark --font duo|quattro|mono --size 18 --focus off|sentence|paragraph [--typewriter] --chrome on|off --text <file.md> --caret "<needle>"|N|end [--mouse] [--typing] [--select a,b] [--nocaret]`
- Latency: `node dev/legacy/tools/latency.mjs --runs 10 --keys 300 --text <file.md> --json out.json`
- Blind pair: `node tools/blind.mjs pair <piece> <ours.png> <theirs.png>` → dev/shots/blind/<piece>/A.png,B.png. `node tools/blind.mjs reveal <piece>` prints which is ours. Critics NEVER run reveal and NEVER read shots outside dev/shots/blind/<piece>/.
- Reference: `dev/ref/ia/REFERENCE.md` (spec sheet, quotes, screenshot table with transcribed text), `dev/ref/ia/shots/`, `dev/ref/ia/fonts/`, `dev/ref/ia/templates/`.
...
1. Crop the reference to just the app/editor region: `node dev/legacy/tools/crop.mjs <ref> <out.png> x y w h [scale]`.
...
Workspace 1 is the user's; a test window goes to a virtual output or to workspace 5. Hyprland 0.56 parses dispatches as Lua, so the old `hyprctl dispatch exec "[rules] cmd"` string form fails; `dev/legacy/bin/quill` carries the working forms:

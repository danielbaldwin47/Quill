...
Screenshots/passages for this piece live in `dev/shots/focus/`.
...
Verification: `dev/shots/theme/themecheck.mjs` (boot on both grounds, live system-follow, toggle,
shortcut, first-paint flash check). Probe/state shots and the passage in `dev/shots/theme/`.
...
Pair for the critic: dev/shots/page/r1-ours.png vs dev/shots/page/r1-theirs.png
...
"garden.", passage in dev/shots/page/passage.md).
...
Probes and raw runs in `dev/shots/latency/` (`probes/*.mjs` are small single-purpose measurements;
`r2-*.json` are the runs quoted in `dev/progress/latency-report.md`).
...
3. **The re-tokenising below an edit is bounded by what is on screen.** When the context entering
   ...
   `dev/shots/latency/probes/micro2.mjs`, `Writer.render(false)` alone, 55k-word document:
   ...
   (`dev/shots/latency/probes/correctness.mjs`).
...
* **caret.js — the caret glides 62 ms behind the letter at normal writing speed, and the glide is
  ...
  keystroke glides. Measured (`dev/shots/latency/probes/caret-glide.mjs`, keydown → the caret's box
...
* **files.js autosave is cheap and correctly debounced.** Instrumented `Storage.setItem` over a
  2,500-keystroke session with pauses: see `autosave` in `dev/shots/latency/r2-long.json`. It fires
...
correctness probe (`dev/shots/latency/probes/correctness.mjs`), and they are **visible to markup**:
...
(`dev/shots/latency/probes/caret-settle.mjs`, `dev/shots/latency/r4-caret-before.json`):
...
`dev/progress/latency-report.md` §6):
...
* **Anything that animates while the writer types costs every keystroke a frame.** The chrome bars'
  ...
  one). Please keep it that way; `dev/shots/latency/probes/animations.mjs` prints the list in one second.

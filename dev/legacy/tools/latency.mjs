...
const DOC = args.doc || args.text || 'dev/shots/latency/doc10k.md';
...
function appFingerprint() {
  try {
    const app = hashApp('.', args.snapshot ? path.join(args.snapshot, 'app') : 'dev/legacy/app');
    let git = null;
    try { git = execSync('git rev-parse --short HEAD', { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim(); } catch (e) {}
    return { ...app, git_head: git, served_from: args.snapshot || 'dev/legacy/app/' };
  } catch (e) { return null; }
}
...
async function typingRun(ctx, opts) {
  ...
  await page.evaluate(() => {
    ...
    // The black-box confirmation (its client rect, frame by frame) is dev/shots/latency/probes/caret-settle.mjs.

...
for (const [theme, font] of [['light','duo'],['dark','quattro'],['light','mono']]) {
  ...
  await p.screenshot({ path: `dev/shots/final-${theme}-${font}.png` });
  await ctx.close();
}
...
await p.goto('http://localhost:4173/', { waitUntil: 'load' }); await p.evaluate(() => document.fonts.ready);
const doc = (await import('node:fs')).readFileSync('dev/shots/latency/doc10k.md','utf8');
...
await p.screenshot({ path: 'dev/shots/final-dark-focus-typewriter.png' });
// library / palette
await p.keyboard.press('Control+K'); await p.waitForTimeout(300); await p.screenshot({ path: 'dev/shots/final-palette.png' }); await p.keyboard.press('Escape');
...
if (results.hasLibrary.length) { await p.evaluate((id) => Writer.run(id), results.hasLibrary[0]); await p.waitForTimeout(500); await p.screenshot({ path: 'dev/shots/final-library.png' }); }
await b.close();

...
dev/ref/ia/
  ...
  dev/shots/                49 real app screenshots (App Store, Microsoft Store, ia.net)
  dev/shots/mac-native/     76 captures of iA Writer for Mac running (§1.1)
...
| Where | What |
|---|---|
| `dev/ref/ia/shots/mac-native/` | the 76 PNG captures |
| `dev/ref/ia/mac-native/NOTES.md` | how each state was driven and what it measured, state by state |
| `dev/ref/ia/mac-native/VERDICTS.md` | every claim in §§3.5, 4.1, 4.2 and in ADRs 0012–0014, marked confirmed / contradicted / still unknown against the running app, each naming its capture |
| `dev/ref/ia/mac-native/rig/` | the driver that took them (`iarig.py`, `drv.sh`, the per-state scripts) |
| `dev/ref/ia/mac-native/blink-idle.tsv`, `blink-typing.tsv` | the ≈103 Hz blink samples behind §3.5's cadence |
| `dev/ref/ia/mac-native/passage-blocks.md`, `passage-markers.md` | the two passages `dev/ref/sample.md` has not got |

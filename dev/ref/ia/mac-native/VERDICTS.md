...
One row per claim in `dev/ref/ia/REFERENCE.md` §§ 3.5, 4.1 and 4.2 and in
...
`dev/ref/ia/REFERENCE.md` is touched by the branch that carries it; what the spec does with it is
...
| 4.3.5b | Classic indents a first line (open since #261: no frame showed a paragraph after a paragraph) | **contradicted** — and for all four Templates | every line of every paragraph starts on the same column; paragraphs are separated by a gap of 1.94 pitches in Classic, 1.97 in Modern, 1.99 in Manuscript | 23-dark-template-{modern,classic,duo,mono}-short |
| 4.3.8 | *(no claim)* the Templates' em | **now measured** | **Manuscript is the Editor's own grid** — a 25.600 px advance and a 73 px pitch, em **42.667 px = 21.333 pt**, line height 1.711, its `H` run pixel-identical to the Editor control's; #261's 72 px Duo pitch was 1 px out | 23-dark-template-{duo,mono,editor}-em |
| 4.3.9 | *(no claim)* the Templates' cap height | **now measured** | **all four set the same cap height** — 28.46 … 28.51 px of ink across three faces, a 0.05 px spread; only the advance differs | 23-dark-template-*-em |
| 4.3.10 | *(no claim)* Classic's face | **now measured** | **not Source Serif 4** — `n`/`H` advance 0.8108 against IBM Plex Serif's 0.8119 and Source Serif 4's 0.7690. Matching iA's cap height with Source Serif 4 needs em 22.24 pt, matching its H advance 19.96 pt: **11 % apart** | 23-dark-template-classic-em |
...
| The narrowest class is **not** described by that container rule as § State 22 wrote it — **superseded by the margin rule below**, which describes it | at 440 pt the container is 828 px where the window allows 860, and the gutter collapses to one cell | 22-light-narrow-w0440-step05-* |
| **Each size class is fourteen sizes**, Default sixth, five Smaller clicks and eight Bigger ones from it — the wide class's count in all three | floors at a 43 px pitch (960 pt) and a 37 px one (400 pt); ceilings at 150 and 129 | `narrow-419-ladder-960.json`, `narrow-419-ladder-400.json` |
| **Neither narrow ladder is the wide one scaled.** Its cell over the wide class's at the same step wanders with no trend, so no step is derivable from another | **0.8211 … 0.9351** across the middle class and **0.7003 … 0.8431** across the narrowest, each turning several times; § State 22's two readings, 0.885 and 0.865, are both interior | 25-light-narrow-w{0960,0400}-step00 … 13 |
| The **liquid line height is each class's own**, and the same falling curve in all three | `pitch / em` 1.61 → 1.35 (middle), 1.63 → 1.32 (narrowest), against the wide class's 1.73 → 1.374 | 25-light-narrow-* |
| The narrowest class's **side margin follows a rule**: `max(5 pt, round(K − advance_pt))`, `K` = 17.5 pt in a window of 240 … 390 pt and 22.5 pt in one of 391 … 440 | fits all 56 rows — 240, 320, 400 and 440 pt across fourteen steps — exactly, refitted by `measure_419.py` on every run; every term of it is in points, the advance being the class's cell halved | `narrow-419-margins-steps.json` (240, 320, 440) and `narrow-419.json` (400); 25-light-narrow-w{0240,0320,0400,0440}-* |
| There is a **fourth break, at 390/391 pt**, inside the narrowest class. It moves the margin and not the type, and like the class breaks it is the window's width alone | bisected at steps 0, 5 and 8, each bisection kept; the same two widths every time; `K` rises by exactly 5 pt across it | `narrow-419-margins.json` § `breaks` |
| **240 pt is the narrowest window the app allows** | asked for 150, 180, 200 or 220 it hands back bounds of 240; `sweep_narrow.py` assumed it for #344 and this run reads it off the app | `narrow-419-margins.json` § `minimum_width` |
...
| **An all-caps word the dictionary does not hold is not marked**, and neither is a token carrying a digit | `DRAFFT`, `2b` and `Q3` are all left alone; seven words of `dev/ref/spell.md` are marked, not eight | 26-light-spell-s1-rest |
...
| **iA has no stats bar.** What stands at the foot of the window is the **Toolbar**, a format bar of thirteen labels with the counts as one popup at its right end — and by default it is not on the screen at all | the app ships on `View > Toolbar > Fade In/Out`; with the pointer away there is paper to the window's edge, at rest and mid-burst alike | 27-light-stats-fade-at-rest, -fade-typing |
| The bar is **40 pt** tall on **the paper itself**, under a **1 pt** rule | `#f7f7f7` / `#1a1a1a` ground — the § 4.2 papers exactly, not a tint; rule `#dbdbdb` / `#2e2e2e`, 2 px | 27-*-stats-c1-top |
| The counts are **body ink**, not a quieter tier, and **hover lifts them to the accent** rather than darkening them | `#191919` / `#cccccc` at rest (4.2.3, 4.2.4); `#36bffa` off the glyph on hover | 27-light-stats-c1-top, -c5-hover |
| **There is no gutter above the bar**: the page runs to the rule and scrolls under it | the last visible row's ink ends **1 px** above the hairline, and the `-nobar` control puts that same row in the same place | 27-light-stats-c1-top + its control |
| The end of a draft keeps **460 pt** of air below the last row — **48.5 %** of a 949 pt window | last ink row 898, rule 1818, 920 px between | 27-*-stats-c2-end |
| The rule stands **whether or not anything continues under it** | rows 1818–1819 in the document-top and document-end frames alike | 27-light-stats-c1-top, -c2-end |
| The counts' menu is a **choice of one**, ten counts each showing its value with a single ✓ | Characters, Without Spaces, **Words**, Sentences, Reading Time, Speaking Time, Tasks, % Human, % AI, % Reference | 27-light-stats-c4-menu, -c4-menu-scrolled |
| A held selection is **counted, and marked as counted** | the figure reads the selection's own count and carries the selection fill `#cbedf7` behind it | 27-light-stats-c6-selection |
| Pinned with Always Show the bar **does not dim while the keys move**, and the counts **update after a pause of about 1.2 s** | a mid-burst frame holds the same labels and the same `#191919`; typing took 0.144 s and the counts first moved **1.176 s after the last key** | 27-light-stats-o1-typing, `stats-381.json` § observations |
| **`View > Toolbar > Stats Only` could not be set from this rig** — still unknown | four ways of pressing it report success, leave `Default` checked and change no pixel | — |
| **4.2.13's library list is measured** (#379): the pane is **two columns and three grounds** | Organizer `#eaebeb` / `#1a1c1b`, File List `#fcfcfc` / `#151515`, the page `#f7f7f7` / `#1a1a1a` beside them; the divider is a 2 px change of ground, not a rule | 28-light-library-l1-rest, 28-dark-library-l1-rest |
| The pane is **360 pt** wide and **drags** | dragged from the column the pane's ground gives way at: **360 pt → 500 pt**, and back to 360. Organizer 129.5 pt, File List 230.5 pt. Two earlier passes aimed 8 pt and 915 pt off the divider and read "does not drag" from a drag that never happened | 28-light-library-o1-dragged, -o1-restored |
| A file row's pitch is **68 pt with excerpts** and **32 pt without**; its excerpt is **two lines**, the title run into the first words | 136 px and 64 px | 28-light-library-l1-rest, -l7-bare |
| The **date and the excerpt share one grey**, and it is **darker** than the editor's dim tier rather than paler | `#999999` / `#757575` against 4.2.5's `#c6c4c2` and 4.2.6's `#707070`; the name is the body ink | 28-*-library-l1-rest |
| **The selected row is an accent bar, not a fill** | **6 px = 3 pt** at the File List's left edge, `#36bffa`, the row's full height — 128 px with excerpts, 56 px without; a right-clicked row is outlined instead | 28-light-library-l1-rest, -l7-bare, -l4-row-menu |
| **The search field is at the foot** of the list, reads `Filter`, and its prompt is **`#7e7e7e`** | on the list's `#fcfcfc`; Quill's is `#BCBCBC`, which is what filed #379 | 28-light-library-l1-rest |
| The Organizer's four section heads are **`#7f8080`**, and an **empty section carries prose** | `Locations`, `Favorites`, `Smart Folders`, `Hashtags`; *Drag folders and files here for quick access*, *Write #tags to group files* | 28-light-library-l1-rest |
| Under Navigation **Tree** a folder **expands in place**, its children indented | `Drafts` turns `›` into `⌄` and puts its two files below it; there is no stepping-in and no way back to find | 28-light-library-l3-folder |
| The **Sort control's menu is the Library pane of Settings**, put on the pane | Date Modified / Created / Name / Extension, Oldest or Newest on Top, Pin Folders to Top, Show Date, Show Text Excerpts, Navigation | 28-light-library-l6-sort-menu |
| **A Favorite is made from the row's own menu** | `Favorite` sits between `Get Info` and `Duplicate` | 28-light-library-l4-row-menu |
| **A hovered row draws nothing**: selection is a bar and hover is nothing | the pane differs from its resting frame only at the search field, which lifts to `#a1d5f5` with its prompt at `#777777` | 28-light-library-l5-hover |
| The search field matches **names and contents together**, and the sort pill becomes **Sort by Search Relevance** while a query stands | `sea` leaves three rows of twelve — one by name, two by contents; `the`, which no name holds, leaves eleven | 28-light-library-l2-search, -o2-search-the |
| **A result row is an ordinary row** — no snippet around the match and no mark on it | same icon, name, date and two-line opening excerpt | 28-light-library-l2-search |
| The separator runs **x 340 … 691** — inset 74 px from the list's left edge and 19 px from its right | `#ededed` light, `#212121` dark | 28-*-library-l1-rest |
| **A file could not be made a Favorite from this rig** — the Favorites row is still unknown | a row's context menu is **not in the accessibility tree**: zero process menus with it open, a click and an `AXPress` both fail, and an arrow-key walk left the Organizer's ink unchanged to the pixel | 28-light-library-l4-row-menu |

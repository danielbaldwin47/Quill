...
MAN = Path("dev/ref/ia/mac-native/capture-2026-09-10.json")
...
def narrow():
    ...
    rows = sorted(json.load(open("dev/ref/ia/mac-native/narrow-344.json")),
                  key=lambda r: (r["limit"], r["step"], r["width_pt"]))
    print("\n#344 — the Editor as the window narrows (Mono, light, dev/ref/sample.md)")
    ...
    edge = json.load(open("dev/ref/ia/mac-native/narrow-344-threshold.json"))
...
def templates():
    ...
    tem = json.load(open("dev/ref/ia/mac-native/templates-343.json"))

...
`library_fixture_379.py --add` puts `dev/shots/oracle/library/` without its
...
a thing that exists.
    ...
    .venv-rig/bin/python3 dev/ref/ia/mac-native/rig/run_library_379.py <raw-dir> <norm-dir>
...
def main():
    ...
    out = {"rig": dict(region_pt=list(REGION), state=28, ticket=379,
                       ...
                       fixture="dev/shots/oracle/library without manifest.json, added to whatever "
    ...
    json.dump(out, open(os.path.join("dev", "ref", "ia", "mac-native", "library-379.json"), "w"), indent=1)

...
above it is read against a page that has no bar under it at all.
    ...
    .venv-rig/bin/python3 dev/ref/ia/mac-native/rig/run_stats_381.py <raw-dir> <norm-dir>
...
def main():
    ...
    kept_path = os.path.join("dev", "ref", "ia", "mac-native", "stats-381.json")
    if only_observe:
    ...
    observe(out, where)
    json.dump(out, open(os.path.join("dev", "ref", "ia", "mac-native", "stats-381.json"), "w"), indent=1)

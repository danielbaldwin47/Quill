#!/bin/bash
# run.sh <label> <bench args...>: wait for an idle machine, run the bench, keep its log.
cd /home/diggle/repos/quill/.claude/worktrees/bench-489 || exit 9
label=$1; shift
out=/home/diggle/.claude/jobs/1a4897df/tmp/runs
mkdir -p "$out"
...
cat "$out/$label.out"
node /home/diggle/.claude/jobs/1a4897df/tmp/phases.mjs "$out/$label.log"

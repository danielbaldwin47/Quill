...
the switch itself moves no pixels.
    ...
    python3 dev/shots/caret/147/jobs147.py <jobs.json> [<unpatched binary>]
...
DEFAULTS = {
    "text": "dev/ref/sample.md", "w": 1440, "h": 900, "theme": "light", "font": "duo",
...
# (`dev/shots/oracle/states.json`, `pieces.caret`), so they pair with the frozen
...
# before it in cell 9.
JUMP = {"text": "dev/shots/caret/147/jump.md", "font": "mono", "chrome": "off", "scroll": 0}
VIEWS = {
...
for view, flags in VIEWS.items():
    jobs.append({"bin": BASELINE, "out": f"dev/shots/caret/147/s1base-{view}.png",
                 "flags": flags, "active": flags["active"]})
for shape in (1, 2, 3, 4, 5):
    for view, flags in VIEWS.items():
        jobs.append({"bin": PATCHED, "out": f"dev/shots/caret/147/s{shape}-{view}.png",
                     "flags": flags, "active": flags["active"],

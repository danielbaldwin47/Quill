...
Wine, and `dev/ref/ia/mac-native/` — iA Writer for Mac running natively — decides where the two differ
...
`dev/ref/ia/mac-native/rig/` is its Mac port.*
...
whose own evidence is under `dev/shots/caret/147/` and whose measurements the tables below carry over.

The new thing here is the opponent. Every earlier reading of iA's caret came from the eight
Microsoft Store stills in `dev/ref/ia/shots/`, in all of which the caret sits at the end of the text —
...
off both of a selection's ends: `dev/shots/caret/mac/` is what those states look like now. They are kept
...
Device pixels at scale 2, from `measure.py`'s repo-side twin in `dev/shots/caret/147/measure147.py`.
...
`dev/progress/rounds/caret-r<N>.json` into the record either way, and a loss on a Piece already won
...
## Reproducing
    ...
    python3 dev/shots/caret/ia/jobs.py /tmp/jobs.json
    node dev/shots/caret/ia/shoot.mjs /tmp/jobs.json
    python3 dev/shots/caret/147/measure147.py dev/shots/caret/ia/ours-*.png

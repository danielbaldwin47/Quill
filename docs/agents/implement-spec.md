...
- **Worktrees** are `docs/agents/worktree.md` § An orchestrator and its forks: the orchestrator stays in its checkout, makes each ticket's worktree off the spec branch by absolute path, and addresses one with `git -C <absolute path>` — a `cd` into a worktree persists into every later Bash call, and three of four orchestrators that did it lost calls to a reset cwd (2026-09-11).
- **The orchestrator merges each ticket branch itself**, `git merge` in the spec worktree; a merger subagent is a context hop and nothing else. A hedge in a fork's report ("rename if review objects") is settled at that merge rather than left for `/code-review`. After every merge and before its commit, `cargo clippy --all-targets -q -- -D warnings` and `cargo test`: a semantic conflict survives a clean textual merge (#263's `band` signature; the one orchestrator that tested after each of eleven merges hit no surprise).

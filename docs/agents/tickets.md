...
- Tests: `cargo test` runs with no display (`docs/agents/gate.md` § Commit tier), so an acceptance criterion asserts a pure function, a model or a file, and reads the widget from a `--deterministic` shot; a criterion that needs a window is a Hand test step.
- Gate tooling the ticket's judge needs (an unfrozen oracle, a state key no tool serves — `tools/gate judge <piece>` exits 3 for both) is its own ticket, before the first judged child.

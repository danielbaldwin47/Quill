...
Either way the spec closes on the owner's `hand test: pass`, by `docs/agents/gate.md` § Feature tier. The agent's last comment on a spec ends with a **Hand test** section: the install step (`docs/agents/hand-tests.md`), then the numbered "do X, see Y" steps written out in full (the `docs/agents/hand-tests.md` checklist merged with the spec's additions), so the owner tests from that comment alone. The last child ticket names itself as the one that writes it. The install step names the worktree to build from (`cd /home/diggle/repos/quill/.claude/worktrees/<spec> && makepkg -si`): `makepkg` in `~/repos/quill` builds `main`, and #318's Hand test, written as `git checkout <branch>`, had the owner test `main` and lose a build cycle (2026-09-11). A failed step is commented on the spec and returns it to the agent.

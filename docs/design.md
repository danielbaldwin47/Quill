...
|---|---|---|---|---|
| Caret column | 6 px bar **centred** on the advance boundary, 3 px each side | left edge a nudge past the boundary (`caret.js`) | **centred on the boundary** — sharpens ADR 0013, whose bar has its left edge there; `Editor::bar()` moves half a bar left | 0013.1–0013.3; `01-caret-offset-*`. What #147 reports as "sits on the leading glyph" — agreed, and `caret/caret` is judged against `01-light-caret-midword` |

...
| Idle fill | `#464646` dark, `#dcdcdc` light — ink over paper at ≈ 0.25 / 0.12 alpha | ink at a lower alpha (`theme.css`) | **ink-alpha at the oracle's strength** | 4.2.11, 0014.9; `07-*`. Alpha survives a palette swap where a hex does not |
...
| What hangs | **headings only**, `#…` at level + 1 cells; `>`, `-`, `1.` sit on the body column; no quote rule | nothing (a textarea cannot); a 2 px quote rule (`markup.css`) | **headings only, no quote rule** | 4.1.10–4.1.13; `14-gutters`, `14-blocks` |
...
| Text sizes | **14 steps** (0–13), em 14.50 … 62.58 pt, default **step 5 = 21.33 pt** | integer px, default 20 (`type.css`, `chrome.js`) | **the oracle's ladder**: `size` becomes the step index, its em in logical px is the ladder's pt value (macOS points are logical px), so the default is 21.33 logical px, 42.67 device px at scale 2; `--size` and `states.json` carry the step | 4.1.2; NOTES § 11 |
| Line pitch | `pitch / em` falls **1.732 → 1.374** over the ladder, 1.711 at the default | a linear clamp (`type.css`) | **the ladder's pitch per step**, NOTES § 11, within 1 device px at every step; a fitted curve only if the ladder is ever extended | 4.1.3–4.1.4 |
...
it omits, or a file that is missing is the built-in; the file is watched like `settings.toml`, and
tolerates keys it does not know. `theme = auto|light|dark` and the portal still choose the scheme.
The Gate judges the built-ins; `--theme` pins them.

...
mod tests {
    ...
    #[test]
    fn center_headings_alone_centres_them_under_every_template() {
        ...
        assert!(
            !modern_off.contains("text-align: center;"),
            "Modern is no exception: {modern_off}"
        );
        let modern_on = page(&sample(), &built("modern"), on);
        assert!(modern_on.contains("text-align: center;"), "{modern_on}");
    }

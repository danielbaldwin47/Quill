...
mod tests {
    ...
    /// Both are steps off a role, and both move when the role does: the light
    /// pair below are what the Design oracle's ink, dimmed grey and paper make
    /// of them, and neither is a number anyone chose.
    ///
    /// The near tier itself is superseded — `docs/design.md` row Dim tiers gives
    /// each ground one tier, and [ADR 0015] says so — but `focus.rs` still
    /// paints it, so [`near_lift`] stays until the ticket that lands that row
    /// takes both away. What this asserts meanwhile is that [`Colour::lift`]
    /// still steps along the line it is given.
    ///
    ...
    fn the_near_tier_and_the_resting_marker_are_computed_rather_than_written() {
        for (scheme, ink_near) in [(Scheme::Light, "#acaaa9"), (Scheme::Dark, "#888888")] {
            let colours = Colours::of(scheme);
            ...
            assert_eq!(near.to_hex(), ink_near, "{scheme:?} ink-near");
        }

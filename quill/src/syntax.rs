//! Syntax highlight's source mapping and retained, block-relative colours.
...
impl Mapping {
    ...
    fn source_spans(&self, tokens: Tokens) -> Tokens {
        let mut out = Vec::new();
        let mut first = 0;
...
impl Syntax {
    /// The master switch, including before a worker has ever been started.
    ...
    pub(crate) fn configure(&mut self, settings: SyntaxHighlight, document: &Document) -> bool {
        ...
        let reset = settings.enabled != self.enabled();
    ...
    /// Carries kept colours over the splice before any Editor redraw.
    ...
    pub(crate) fn spans_in(&self, document: &Document, at: &Range<usize>) -> Tokens {
        ...
        for index in first..=last {
            ...
            {
                tokens.push((origin + span.start..origin + span.end, *category));
            }
        }
        tokens
    }

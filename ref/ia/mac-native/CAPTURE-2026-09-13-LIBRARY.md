...
pane is drawn to `legacy/app/css/files.css`, whose numbers the JavaScript app measured off iA's
...
decided. `defaults.font` in `shots/oracle/states.json` is `duo` and these frames are Mono, so such

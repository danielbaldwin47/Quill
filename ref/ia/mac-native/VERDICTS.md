...
| **Neither narrow ladder is the wide one scaled.** Its cell over the wide class's at the same step wanders with no trend, so no step is derivable from another | 0.9195 … 0.8888 across the middle class, dipping to 0.8211 at step 6; 0.7829 … 0.7790 across the narrowest, dipping to 0.7003 at step 7 | 25-light-narrow-w{0960,0400}-step00 … 13 |
...
| The narrowest class's **side margin follows a rule**: `max(5 pt, round(K − advance))`, `K` = 17.5 pt in a window of 240 … 390 pt and 22.5 pt in one of 391 … 440 | fits all 56 rows — 240, 320, 400 and 440 pt across fourteen steps — exactly | 25-light-narrow-w{0240,0400,0440}-*, `narrow-419-margins-steps.json` |
| There is a **fourth break, at 390/391 pt**, inside the narrowest class. It moves the margin and not the type, and like the class breaks it is the window's width alone | bisected at steps 0, 5 and 8; the same two widths each time; `K` rises by exactly 5 pt across it | `narrow-419-margins.json` |
| **240 pt is the narrowest window the app allows** | asked for 150, 180, 200 or 220, it returns 240 | `narrow-419-margins.json` |
| With the margin the class's own, the container rule holds **everywhere**: `min((limit + 14) cells, window − 2 × margin)`, the margin 5 pt in the middle and wide classes | in the narrowest class the first term never wins — 78 cells of the smallest type wants 1062 px where the class's widest window allows 860 | 25-light-narrow-* |
| The container is **centred to the half pixel**; where the leftover is odd the extra device pixel falls on the right | 20 px left against 22 right at step 1 in a 480 px window; 74 against 76 at step 5 in a 1920 px one | 25-light-narrow-* |

...
| `sweep_419.py`, `margins_419.py` | #419: the width the narrowest class's margin changes at, bisected the way `sweep_narrow.py` bisects the size classes; and that margin at every step, one width at a time. A margin needs only the container's own left edge, so both shoot select-all frames and both keep them as scratch |

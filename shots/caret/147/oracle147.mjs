...
//
// The frozen oracle under `shots/oracle/caret/` holds the three judged states
...
// writes into the evidence directory rather than `shots/oracle/`, so no oracle
...
// `shots/caret/147/jump.md`, the same offset 10, at mono 20 px with the chrome
...
// shots/caret/147/ -> the repo root.
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../..');
const PORT = 4319;

const flags = {
  text: 'shots/caret/147/jump.md', w: 1440, h: 900, scale: 2, theme: 'light',
...
const server = spawn('node', [path.join(root, 'legacy/tools/serve.mjs'), String(PORT)],
...
try {
  fs.mkdirSync(path.join(root, 'shots/caret/147'), { recursive: true });
  for (const s of states) {
    const out = `shots/caret/147/oracle-${s.name}.png`;
    execFileSync('node', [path.join(root, 'legacy/tools/shoot.mjs'),

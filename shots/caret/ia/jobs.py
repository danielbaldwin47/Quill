...
`selection`, `unfocused`, verbatim from `shots/oracle/states.json`, so they pair
...
the `t` of `test`, with the space before it in cell 9.
    ...
    python3 shots/caret/ia/jobs.py <jobs.json> [<binary>]
"""
import json
import sys
...
# `shots/oracle/states.json`'s `defaults`, which a job's flags have to carry
# resolved: `quillArgv` reads a whole state, not a patch on one.
DEFAULTS = {
    "text": "ref/sample.md", "w": 1440, "h": 900, "scale": 2, "theme": "light",
...
JUMP = {"text": "shots/caret/ia/jump.md", "font": "mono", "chrome": "off", "scroll": 0}
...
jobs = [{"bin": BIN, "out": f"shots/caret/ia/ours-{view}.png",

...
out = {
 ...
 "bar_note": ("iA Writer publishes no latency numbers at all. The bar is third-party: (1) 32.5 +/- 4.0 ms keyboard-to-photon for Sublime Text "
   ...
   "'disconnected' (shots/latency/r4-no-display.txt) and Hyprland runs on a FALLBACK headless output - so scan-out cannot be measured here at "
   ...
   "sd 5.01 against Hume's 4.0, where round 3 was 10.72. Full arithmetic: progress/latency-report.md sections 8 and 9.")
...
out['detail']={
 ...
 "environment": {
   ...
   "display": "NONE. Every DRM connector on all seven cards reads 'disconnected'; Hyprland runs on a FALLBACK headless output. Dump: shots/latency/r4-no-display.txt",
 ...
 "document": {"words": 10062, "chars": 53031, "lines": 432,
   "source": "Alice's Adventures in Wonderland (Project Gutenberg #11), one paragraph per line, italics as Markdown emphasis, an editor's note with a task list, a link, inline code and a fenced code block - shots/latency/doc10k.md"},
 "bar_app_internal": {
 ...
 "caret_ms": {
   "what": "keydown -> the caret's box has stopped moving, black box, reading its client rect every animation frame (shots/latency/probes/caret-settle.mjs)",
 ...
 "report": "progress/latency-report.md",
}
json.dump(out, open('progress/latency.json','w'), indent=2)

...
/// the budget was set on. Not on each regime's row in a run of thirteen, which would say it thirteen
...
///
/// One regime's line and a whole run's line are read against each other — a `--all` run is thirteen
/// of the second under the first — so the shape they share is written once here. What follows the
/// numbers is what differs: a single run names the bars, a regime in a run of thirteen leaves them
...
/// in a run of thirteen — because its numbers read as a fail against the budget to anyone who does
...
///
/// A run of thirteen is read down the left-hand edge, so each regime says the same five numbers in
...
/// and a run of thirteen that passed are not the same evidence, and the line the owner reads should
...
/// thirteen reads "12 of 12 scored regimes" rather than a fail nobody can clear.

...
function usage(to = process.stderr) {
  ...
  regime          which of the sixteen to type; ${HEADLINE} by default
  --all           every one of the sixteen, one line each and one line for the run
...
result file, one regime or sixteen, is marked informational inside and named bench-panel-<regime>-.
`);
}
...
///
/// The stage is the caller's, because opening one costs a compositor output and the sixteen regimes
...
async function benchOne(root, stage, { regime, keys, sessions, warmup, panel }) {
  ...
  return {
    ...
    // of one prints and the lines a run of sixteen prints cannot be assembled two different ways.
    reported: {
...
/// both ask the same question of a whole run, and neither should have to reopen sixteen files and
/// decide for itself which sixteen they were.
function manySaid(root, { ran, chosen, done, warmup, panel }) {
  ...
  // re-deriving a verdict from sixteen result files and hoping it phrases it the same way.
...
async function main(argv) {
  ...
  for (const want of choice.names ?? [name ?? HEADLINE]) {
    ...
    if (!found) {
      process.stderr.write(`gate bench: ${want}: not one of the sixteen regimes `

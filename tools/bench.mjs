...
function usage(to = process.stderr) {
  ...
  regime          which of the thirteen to type; ${HEADLINE} by default
  --all           every one of the thirteen, one line each and one line for the run
...
result file, one regime or thirteen, is marked informational inside and named bench-panel-<regime>-.
`);
}
...
// window size, the 10k-word document, and the caret, Focus and Live the regime asks for, because
// where the writer is, how much of the draft is dimmed and whether the markup is folded away are
// three of the things a regime varies.
...
///
/// The stage is the caller's, because opening one costs a compositor output and the thirteen regimes
...
async function benchOne(root, stage, { regime, keys, sessions, warmup, panel }) {
  ...
  return {
    ...
    // of one prints and the lines a run of thirteen prints cannot be assembled two different ways.
    reported: {
...
/// both ask the same question of a whole run, and neither should have to reopen thirteen files and
/// decide for itself which thirteen they were.
function manySaid(root, { ran, chosen, done, warmup, panel }) {
  ...
  // re-deriving a verdict from thirteen result files and hoping it phrases it the same way.
...
async function main(argv) {
  ...
  for (const want of choice.names ?? [name ?? HEADLINE]) {
    ...
    if (!found) {
      process.stderr.write(`gate bench: ${want}: not one of the thirteen regimes `

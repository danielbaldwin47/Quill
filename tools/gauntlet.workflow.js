export const meta = {
  ...
  phases: [
    ...
    { title: 'Record', detail: 'reveal + write dev/progress/rounds/<piece>-r<N>.json', model: 'haiku' },
  ],
}
...
const PIECES = [
  ...
  { id: 'latency', title: 'Latency', judge: 'measured numbers and methodology: keystroke-to-paint p50/p99, startup to editor-ready, with a large document; is the measurement honest and does it beat the bar (Sublime 32.5 ms keyboard-to-photon; ≤5 ms internal input→paint; iA publishes no numbers)', build: 'harden dev/legacy/tools/latency.mjs (count every keystroke, FCP in headless, real 10k-word document, p50/p95/p99, also measure via CDP tracing frame presentation), a dev/legacy/bin/quill launcher (chromium --app) with cold-start measurement, profile and remove any per-keystroke cost in core.js/render, write dev/progress/latency.json with keys {keystroke_p50, keystroke_p99, startup_ready, startup_note, bar_keystroke, bar_note, method, at} and dev/progress/latency-report.md', states: 'not a screenshot piece — the critic reads dev/progress/latency.json and dev/progress/latency-report.md' },
]
...
function builderPrompt(p, round, feedback) {
  return `You are the BUILDER for the piece "${p.title}" (id: ${p.id}) of Quill, a long-form writing environment that must beat iA Writer. Round ${round}. Work in ${ROOT} (git repo; the static server is already running at http://localhost:4173/ — if it is not, start it with \`node dev/legacy/tools/serve.mjs 4173 &\`).

FIRST read, in this order: dev/legacy/BRIEF.md, dev/legacy/NOTES.md, dev/legacy/app/js/core.js, then your own files, then dev/ref/ia/REFERENCE.md (the spec sheet, quotes and the screenshot table with transcribed passages). Look at the relevant iA reference screenshots in dev/ref/ia/shots/ with the Read tool — you must compare against the real thing, not a description.
...
Rules:
- Edit only the files you own (dev/legacy/BRIEF.md table). Minimal, noted changes elsewhere go in dev/legacy/NOTES.md under "## ${p.id}". Never break glyph alignment between #input and #mirror; never change glyph advance widths per token; never add per-keystroke heavy work. Keep \`node --check\` passing on any JS you touch, and check both themes and all three fonts still render (a quick shoot in dark + light).
...
- Verify visually: \`node dev/legacy/tools/shoot.mjs\` (see dev/legacy/BRIEF.md flags) at EXACTLY the pixel size of the reference crop, with the same transcribed passage (write it to a file under dev/shots/${p.id}/ e.g. dev/shots/${p.id}/passage.md), same theme/mode/caret. View both images with the Read tool and compare like a typographer. Iterate until you honestly believe a demanding writer would pick ours blind. Be ruthless with yourself; the critic is.
- Then produce the pair for the critic: crop the reference to its app region with dev/legacy/tools/crop.mjs (record the crop box), render ours to dev/shots/${p.id}/r${round}-ours.png, save the crop to dev/shots/${p.id}/r${round}-theirs.png, and run \`node tools/blind.mjs pair ${p.id} dev/shots/${p.id}/r${round}-ours.png dev/shots/${p.id}/r${round}-theirs.png\`. NEVER run \`blind.mjs reveal\`.
- Finally commit your changes: \`git add legacy dev/shots/${p.id} && git commit -m "${p.id}: round ${round} — <what changed>"\` (if the commit races with another builder's, retry once; if the index is locked, wait 2s and retry).
${p.id === 'latency' ? `\nLATENCY SPECIFICS: this piece is judged on numbers, not screenshots. Produce dev/progress/latency.json and dev/progress/latency-report.md (methods, environment: headless Chromium 151 on Linux, CPU, document size, N, how frames were detected, caveats). Also try a headed measurement through the dev/legacy/bin/quill launcher if a display is available (check $WAYLAND_DISPLAY / $DISPLAY; if none, say so). For the pair, set oursShot to dev/progress/latency.json and theirsShot to dev/ref/ia/REFERENCE.md and skip blind pairing (paired:false). Do not delete previous shots.` : ''}
...
Two screenshots of writing apps, unlabeled, at the same viewport: ${ROOT}/dev/shots/blind/${p.id}/A.png and ${ROOT}/dev/shots/blind/${p.id}/B.png. Look at BOTH with the Read tool (view each at least twice; zoom into details by cropping with \`node dev/legacy/tools/crop.mjs <img> /home/diggle/.claude/jobs/e9d93e91/tmp/crop-${p.id}-r${round}-<n>.png x y w h 2\` and viewing the crop, e.g. the caret, a heading marker, a line of body text). Do NOT read any other files or images, do NOT run \`blind.mjs reveal\`, do NOT try to work out which app is which — judge only what you see. Ignore differences in the passage's wording if any; judge the design and the writing experience it implies for this piece only. If the two images are not the same pixel size, set sameViewport=false and still judge, but say so.
...
function latencyCriticPrompt(p) {
  return `You are a HARSH performance reviewer for a writing app. Read ${ROOT}/dev/progress/latency.json and ${ROOT}/dev/progress/latency-report.md, and the "Performance" section of ${ROOT}/dev/ref/ia/REFERENCE.md (the bar: iA Writer publishes no numbers; best native editors ≈32.5 ms keyboard-to-photon (Sublime, macOS), ≤5 ms internal input→paint; Dan Luu's 2 ms perception threshold). Also read dev/legacy/tools/latency.mjs and skim dev/legacy/app/js/core.js for per-keystroke cost. Decide: do OUR measured numbers beat that bar with HONEST methodology (large document, every keystroke counted, paint actually detected, p99 not just p50, startup measured to editor-ready and ideally cold launch of the real launcher)? Poke holes: what is unmeasured, what is optimistic, what would a sceptic say? pick='ours' only if the numbers beat the bar AND the methodology holds up; otherwise 'theirs'. gapOurs = the single biggest hole or shortfall to fix next. verdict = 2–4 blunt sentences.`
}

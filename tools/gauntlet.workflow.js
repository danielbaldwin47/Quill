...
const PIECES = [
  ...
  { id: 'latency', title: 'Latency', judge: 'measured numbers and methodology: keystroke-to-paint p50/p99, startup to editor-ready, with a large document; is the measurement honest and does it beat the bar (Sublime 32.5 ms keyboard-to-photon; ≤5 ms internal input→paint; iA publishes no numbers)', build: 'harden legacy/tools/latency.mjs (count every keystroke, FCP in headless, real 10k-word document, p50/p95/p99, also measure via CDP tracing frame presentation), a legacy/bin/quill launcher (chromium --app) with cold-start measurement, profile and remove any per-keystroke cost in core.js/render, write progress/latency.json with keys {keystroke_p50, keystroke_p99, startup_ready, startup_note, bar_keystroke, bar_note, method, at} and progress/latency-report.md', states: 'not a screenshot piece — the critic reads progress/latency.json and progress/latency-report.md' },
]
...
function builderPrompt(p, round, feedback) {
  return `You are the BUILDER for the piece "${p.title}" (id: ${p.id}) of Quill, a long-form writing environment that must beat iA Writer. Round ${round}. Work in ${ROOT} (git repo; the static server is already running at http://localhost:4173/ — if it is not, start it with \`node legacy/tools/serve.mjs 4173 &\`).

FIRST read, in this order: legacy/BRIEF.md, legacy/NOTES.md, legacy/app/js/core.js, then your own files, then ref/ia/REFERENCE.md (the spec sheet, quotes and the screenshot table with transcribed passages). Look at the relevant iA reference screenshots in ref/ia/shots/ with the Read tool — you must compare against the real thing, not a description.
...
Rules:
- Edit only the files you own (legacy/BRIEF.md table). Minimal, noted changes elsewhere go in legacy/NOTES.md under "## ${p.id}". Never break glyph alignment between #input and #mirror; never change glyph advance widths per token; never add per-keystroke heavy work. Keep \`node --check\` passing on any JS you touch, and check both themes and all three fonts still render (a quick shoot in dark + light).
...
- Verify visually: \`node legacy/tools/shoot.mjs\` (see legacy/BRIEF.md flags) at EXACTLY the pixel size of the reference crop, with the same transcribed passage (write it to a file under shots/${p.id}/ e.g. shots/${p.id}/passage.md), same theme/mode/caret. View both images with the Read tool and compare like a typographer. Iterate until you honestly believe a demanding writer would pick ours blind. Be ruthless with yourself; the critic is.
- Then produce the pair for the critic: crop the reference to its app region with legacy/tools/crop.mjs (record the crop box), render ours to shots/${p.id}/r${round}-ours.png, save the crop to shots/${p.id}/r${round}-theirs.png, and run \`node tools/blind.mjs pair ${p.id} shots/${p.id}/r${round}-ours.png shots/${p.id}/r${round}-theirs.png\`. NEVER run \`blind.mjs reveal\`.
- Finally commit your changes: \`git add legacy shots/${p.id} && git commit -m "${p.id}: round ${round} — <what changed>"\` (if the commit races with another builder's, retry once; if the index is locked, wait 2s and retry).
...
Two screenshots of writing apps, unlabeled, at the same viewport: ${ROOT}/shots/blind/${p.id}/A.png and ${ROOT}/shots/blind/${p.id}/B.png. Look at BOTH with the Read tool (view each at least twice; zoom into details by cropping with \`node legacy/tools/crop.mjs <img> /home/diggle/.claude/jobs/e9d93e91/tmp/crop-${p.id}-r${round}-<n>.png x y w h 2\` and viewing the crop, e.g. the caret, a heading marker, a line of body text). Do NOT read any other files or images, do NOT run \`blind.mjs reveal\`, do NOT try to work out which app is which — judge only what you see. Ignore differences in the passage's wording if any; judge the design and the writing experience it implies for this piece only. If the two images are not the same pixel size, set sameViewport=false and still judge, but say so.

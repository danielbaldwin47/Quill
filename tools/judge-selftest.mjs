...
import { OPPONENTS, decisive, nextRound, opponentName, round, wonBefore, wonState } from './rounds.mjs';
...
// Round 7's own pairs for the two states that are still measured: each one's shot and its
// `--spell off` reshoot, as `tools/gate judge spell` committed them — round 7 because #416 changed
// what is on the page, the dots for the wave and seven marks for eight. Variants replace pixels in
// those shots, never app internals. The eight states judged on a `mac-native` crop have no reshoot
...
// The eight states #400's capture holds a frame of, and the frame each names. The other two — the
// caret parked in a marked word, and the "no dictionary" state — are the two it shot no frame of,
// and they stay measured off ours' own pixels (ADR 0017).
const SPELL_FRAMES = {
  'on-light': 'mac-native-26-light-spell-s1-rest.png',
  'on-dark': 'mac-native-26-dark-spell-s1-rest.png',
  'focus-light': 'mac-native-26-light-spell-s2-focus.png',
  'focus-dark': 'mac-native-26-dark-spell-s2-focus.png',
  'syntax-light': 'mac-native-26-light-spell-s3-syntax.png',
  'syntax-dark': 'mac-native-26-dark-spell-s3-syntax.png',
  'select-light': 'mac-native-26-light-spell-s4-selection.png',
  'select-dark': 'mac-native-26-dark-spell-s4-selection.png',
};

ok('eight Spell states are judged on #400’s frames and the two it shot none of are measured', () => {
  ...
  for (const state of resolveStates(states, 'spell')) {
    ...
    if (frame) {
      ...
      assert.equal(state.opponent.capture, frame, state.name);
      // The capture is Mono, and a state naming one is shot in Mono whatever the defaults say, so
      // the two grids compare cell for cell.
      assert.equal(state.flags.font, 'mono', state.name);
      continue;
    }
    ...
    // Seven marks on the passage — a parked caret withholds none, and `DRAFFT` is an all-caps word
    // the dictionary does not hold and is left alone — and none with no dictionary.
    const words = { 'missing-light': 0 }[state.name] ?? 7;
...
ok('a missing mark, a mark where none is wanted and a mark in another colour are each refused', () => {
  const on = spellPair('caret-light');
  // No mark at all: the reshoot against itself.
  assert.match(assertState(spellRule('caret-light'), { dim: on.lit, lit: on.lit }).why, /changed no pixel/);
  // A parked caret that hid its word's mark: the caret state's page with one mark fewer than the count.
  assert.match(assertState({ ...spellRule('caret-light'), words: 9 }, on).why, /holds 7 marks \(.*\) and the state expects 9/);
  // Marks on a page with no dictionary.
  assert.match(assertState(spellRule('missing-light'), on).why, /drew \d+ pixels of the spell Role/);
  // The marks repainted in the link rule's blue: the dots drawn from the wrong Role.
  ...
  assert.match(assertState(spellRule('caret-light'), { ...on, dim: encodePng(dim) }).why, /spell Role in 0 of/);
  ...
  for (const bad of [{ words: -1 }, { words: 1.5 }, { theme: 'sepia' }, { status: 'yes' }, { status: true }, { runs: 2 }]) {
    assert.throws(() => validate({ kind: 'spell', theme: 'light', words: 7, ...bad }), /spell/, JSON.stringify(bad));
  }
});
...
ok('every judged state that draws a determined caret is held to one, and no other state is', () => {
  ...
  assert.deepEqual(exempt, [
    ...
    // since #400's captures carry no bar. `spell/caret-light` still places the caret, because it
    // proves a parked caret keeps its word's mark.
...
function recordedRound(n, name, winner, { cropped = false, carried = null, pick = 'A' } = {}) {
  const state = {
    ...
    oursWas: 'A', pick, winner, margin: 'slight', sameViewport: true, gap: 'g', gapTheirs: 't', verdict: 'v', secondary: [],
...
ok('a state was won when a critic once gave it to ours against the same kind of opponent', () => {
  const r3 = recordedRound(3, 'sentence', 'ours');
  const r4 = recordedRound(4, 'sentence', 'theirs');
  assert.equal(wonState([r3, r4], 'sentence', false).round, 3, 'the latest round a critic gave the state to ours, not the latest round');
  assert.equal(wonState([r4], 'sentence', false), null, 'a state never won is not one');
  assert.equal(wonState([r3, r4], 'paragraph', false), null, 'another state\'s win is not this one\'s');
  // A win against the Parity oracle's whole window does not vouch for the state once it names a
  // mac-native crop: the pair is a different question (ADR 0015), and the caret's selection went
  // from won against dev/legacy/ to judged against iA the day #168 gave it a crop.
  assert.equal(wonState([r3], 'sentence', true), null);
  assert.equal(wonState([recordedRound(5, 'sentence', 'ours', { cropped: true })], 'sentence', true).round, 5);
  assert.equal(wonState([recordedRound(5, 'sentence', 'ours', { cropped: true })], 'sentence', false), null);
  // A carried win was a critic's, and points at the round whose critic looked.
  assert.equal(wonState([recordedRound(6, 'sentence', 'ours', { carried: 3 })], 'sentence', false).round, 3);
  // An assertion has no critic and no pick, so it never makes a state once-won; nor does a gauntlet
  // round with no states.
  assert.equal(wonState([recordedRound(7, 'sentence', 'ours', { pick: null })], 'sentence', false), null);
  assert.equal(wonState([{ piece: 'x', round: 1, winner: 'ours' }], 'sentence', false), null);
});
...
ok('a once-won state\'s loss is a second critic\'s question, and is settled on both answers', () => {
  ...
  const agreed = settle(lost, second);
  assert.equal(agreed.winner, 'theirs', 'both said theirs: lost');
  assert.equal(agreed.margin, 'slight', 'at the first critic\'s margin');
  ...
  assert.equal(split.gap, 'first g', 'the loss the first critic read is still on record');

  // A round settled that way is still one every reader can take: the Piece is ours when every
  // state is, and a state carried later copies the second answer with the rest of the verdict.
  const written = round({ piece: 'x', number: 5, judged: [split], opponent: 'oracle', build: {}, oracle: '', note: '', at: 'now' });
  assert.equal(written.winner, 'ours');
  assert.equal(written.states[0].second.winner, 'ours');
  assert.deepEqual(carriedVerdict(split).second, split.second, 'a carried verdict carries the second critic\'s answer too');
  assert.ok(!('second' in carriedVerdict(lost)), 'and a verdict one critic gave carries no empty one');
  assert.deepEqual(Object.keys(carriedVerdict(lost)).sort(), [...VERDICT_KEYS].sort());
});
